import winsound
"""这个是自动答题的函数"""

def text(top_p,temperature, presence_penalty,frequency_penalty):
    # -*- coding: utf-8 -*-
    import json
    import time
    import requests
    with open("answer.json", "w", encoding="utf-8"):
        i =1
    # Function to read JSON file

    def readJson(json_name):
        with open(json_name, 'r', encoding='utf-8') as file:
            data = json.load(file)

        myQuestion = ["qwq"]  # 0 index as placeholder
        tiaoshi_num = 1
        for item in data:
            name = item.get('instruction', '默认值')
            myQuestion.append(name)
            print(f"读取json，读到第{tiaoshi_num}个了")
            tiaoshi_num += 1
        return myQuestion

    # ===================配置项目=================================
    """这是key的存放位置"""
    api = ["hk-xwmrry1000057382a22c07aef0d839d6de1793e49c17e3f6",
           ]
    key_num = 0
    model = "gpt-4o-mini"  # Changed to use OpenAI-HK model
    QuestionNumberStart = 1
    yourQuestionNumber = 10
    answer_filename = "answer.json"
    json_name = "wenti.json"

    # Prompt settings
    oneMyPromptSentence = ""
    twoMyPromptSentence = ""

    # ========== API 配置 ==========
    api_key = api[key_num]
    api_url = "https://api.openai-hk.com/v1/chat/completions"

    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {api_key}"
    }

    # ==================
    # call_model 函数开始 (modified for OpenAI-HK API)
    # ==================
    def call_model(prompt, retries=3, key_num=key_num):
        """调用OpenAI-HK API获取模型响应"""
        for attempt in range(retries):
            try:
                data = {
                    "max_tokens": 500,
                    "model": model,
                    "temperature": temperature,
                    "top_p": top_p,
                    "presence_penalty": presence_penalty,
                    "frequency_penalty": frequency_penalty,
                    "messages": [
                        {
                            "role": "system",
                            "content": ""
                        },
                        {
                            "role": "user",
                            "content": f"{oneMyPromptSentence}{prompt}{twoMyPromptSentence}"
                        }
                    ]
                }

                response = requests.post(api_url, headers=headers, data=json.dumps(data).encode('utf-8'))
                response.raise_for_status()  # Raise exception for HTTP errors

                result = response.content.decode("utf-8")
                result_json = json.loads(result)
                res_final = result_json["choices"][0]["message"]["content"]

                return res_final, ""

            except requests.exceptions.HTTPError as e:
                if response.status_code == 429:
                    wait_time = 10 + attempt * 5
                    print(f"⚠️ 请求过快 (429)，第 {attempt + 1} 次重试中... 等待 {wait_time} 秒")
                    time.sleep(wait_time)
                else:
                    print(f"API调用错误: {e}")
                    return "ERROR", ""
            except Exception as e:
                print(f"其他错误: {e}")
                return "ERROR", ""
        return "ERROR", ""

    # ==================
    # 主程序部分
    # ==================
    question_WillInput = readJson(json_name)

    # 准备输出文件
    file_willWrite = open(answer_filename, "w", encoding="utf-8")
    file_willWrite.write("[\n")
    answer_dic = {}

    while QuestionNumberStart <= yourQuestionNumber:
        base_response = call_model(question_WillInput[QuestionNumberStart])
        print(f"【{QuestionNumberStart}】实时输出程序状态-模型回答的答案: {base_response[0]}")
        temp_dict = {"output": base_response[0]}
        file_willWrite.write(json.dumps(temp_dict, indent=4, ensure_ascii=False))

        if QuestionNumberStart != yourQuestionNumber:
            file_willWrite.write(",")
        file_willWrite.write("\n")

        time.sleep(2)  # 降低请求频率
        QuestionNumberStart += 1

    # 完成文件写入
    file_willWrite.write("]\n")
    file_willWrite.close()
    print(f"模型自动回答完毕，答案已存储在文件【{answer_filename}】中")


"""这个是评测的函数"""
def data():
    import json
    from bert_score import BERTScorer

    import nltk
    # nltk.download('wordnet')  # 雨自行添加
    from nltk.translate.bleu_score import sentence_bleu, SmoothingFunction
    from nltk.translate.meteor_score import meteor_score

    from sklearn.feature_extraction.text import TfidfVectorizer
    from sklearn.metrics.pairwise import cosine_similarity

    import torch
    from transformers import GPT2LMHeadModel, GPT2Tokenizer

    import hashlib
    import math

    # =====================参数修改================================
    my_answer_filename = "answer.json"  # 要评测的答案的文件名
    my_json_filename = "wenti.json"  # 用于评测对比的json文件名
    my_num = 10  # 数据数量 用于计算平均指标

    # gene1_final_file(stsb-roberta-large).txt
    # ============================================================

    class SimHash:
        def __init__(self, tokens, hash_bits=64):
            self.hash_bits = hash_bits
            self.hash = self._calculate_hash(tokens)

        def _calculate_hash(self, tokens):
            # 初始化一个长度为 hash_bits 的向量
            v = [0] * self.hash_bits

            for token in tokens:
                # 对每个 token 计算哈希值
                hash_value = self._hash(token)
                # 将哈希值转换为二进制表示
                binary_hash = bin(hash_value)[2:].rjust(self.hash_bits, '0')

                # 更新向量
                for i in range(self.hash_bits):
                    if binary_hash[i] == '1':
                        v[i] += 1
                    else:
                        v[i] -= 1

            # 将向量转换为最终的哈希值
            fingerprint = 0
            for i in range(self.hash_bits):
                if v[i] > 0:
                    fingerprint |= (1 << i)

            return fingerprint

        def _hash(self, token):
            # 使用 hashlib 计算哈希值
            return int(hashlib.md5(token.encode('utf-8')).hexdigest(), 16)

        def distance(self, other):
            # 计算汉明距离
            x = (self.hash ^ other.hash) & ((1 << self.hash_bits) - 1)
            return bin(x).count('1')

    # 加载预训练的 GPT-2 模型和 tokenizer
    perplexitymodel = GPT2LMHeadModel.from_pretrained('gpt2')
    perplexitytokenizer = GPT2Tokenizer.from_pretrained('gpt2')

    def compute_perplexity(text):
        inputs = perplexitytokenizer.encode(text, return_tensors="pt")
        # 计算模型的输出
        outputs = perplexitymodel(inputs, labels=inputs)
        # 获取损失（loss）
        loss = outputs.loss
        # 计算困惑度（Perplexity = exp(loss)）
        perplexity = torch.exp(loss)
        return perplexity.item()

    # 确保下载 punkt 模块，用于分词
    # nltk.download('punkt')

    scorer = BERTScorer(
        lang="en",
        # model_type="microsoft/deberta-xlarge-mnli",
        model_type="albert-large-v2",  # 评测用的模型，记录数据的参数之一
        num_layers=5,
        rescale_with_baseline=True,
        device="cpu",
    )
    print("开始评判")

    # 打开 JSON 文件
    with open(my_json_filename, 'r', encoding='utf-8') as file:
        # 加载 JSON 数据
        data = json.load(file)

    # # 打印读取的数据
    # print(data)

    # 假设你想读取每个对象的 'name' 属性

    array1 = []
    i = 0
    for item in data:
        name = item.get('output', '默认值')  # 使用 get 方法，避免 KeyError
        array1.append(name)
        i += 1
        # print(name)
    i = 0

    precision = 0
    recall = 0
    f1 = 0
    bleu = 0
    Cosine = 0
    perplexity = 0
    Hamming = 0
    METEOR = 0
    num = my_num

    # 使用TF-IDF向量计算余弦相似度
    def tfidf_cosine_similarity(text1, text2):
        # 初始化TF-IDF向量化器
        vectorizer = TfidfVectorizer()
        # 将文本转换为TF-IDF向量
        tfidf_matrix = vectorizer.fit_transform([text1, text2])
        # 计算余弦相似度
        similarity = cosine_similarity(tfidf_matrix[0:1], tfidf_matrix[1:2])
        return similarity[0][0]

    # with open('gene1_final_file(all-MiniLM-L6-v2).txt', 'r', encoding='utf-8') as f:
    # gene1_final_file(stsb-roberta-large).txt

    with open(my_answer_filename, 'r', encoding='utf-8') as f:
        # 加载 JSON 数据
        answer_data = json.load(f)

        # with open(my_answer_filename, 'r', encoding='utf-8') as f:
        #     answer_data = json.load(f)
        for item in answer_data:
            line = item.get('output', '默认值')  # 使用 get 方法，避免 KeyError
            # print(line)

            p = array1[i]
            i += 1
            # print(p)
            # line = line.rstrip('\n')  # 去除换行符
            # print(line)

            references = [p]
            candidates = [line]
            P, R, F1 = scorer.score(candidates, references)
            print(f"Precision: {P.mean():.4f}, Recall: {R.mean():.4f}, F1: {F1.mean():.4f}")
            precision += P.mean()
            recall += R.mean()
            f1 += F1.mean()
            print()

            # candidate = "IutB does not have a subunit structure because it functions as a monomeric protein, meaning it operates effectively as a single unit without the need for multiple subunits. This simplicity allows it to perform its role efficiently, often in transport or binding processes, where a single polypeptide chain is sufficient for its biological activity. The absence of subunits may also reflect evolutionary optimization, reducing the complexity of assembly and regulation while maintaining functionality. Additionally, its structure might be inherently stable without requiring interactions between multiple subunits, ensuring reliable performance in its cellular context.".split()
            # 计算 METEOR 得分
            metorreference = [p.split()]
            metorcandidate = line.split()
            score = meteor_score(metorreference, metorcandidate)
            print("METEOR Score:", score)
            METEOR += score

            tfidf_similarity = tfidf_cosine_similarity(p, line)
            print("TF-IDF Cosine Similarity:", tfidf_similarity)
            Cosine += tfidf_similarity
            # 将字符串分词为单词列表
            reference1 = [p.split()]
            candidate1 = line.split()
            weights = [1, 0, 0, 0]  # 只计算 1-gram 和 2-gram
            # 使用平滑函数
            smoothie = SmoothingFunction().method1
            # 计算 BLEU 分数，使用平滑
            bleu_score = sentence_bleu(reference1, candidate1, weights=weights, smoothing_function=smoothie)
            print(f"BLEU Score: {bleu_score:.4f}")
            bleu += bleu_score

            text = line
            perplexity1 = compute_perplexity(text)
            print(f"Perplexity: {perplexity1}")
            perplexity += perplexity1

            # 分词
            tokens1 = p.split()
            tokens2 = line.split()
            # 计算 SimHash
            simhash1 = SimHash(tokens1)
            simhash2 = SimHash(tokens2)
            # 计算汉明距离
            hamming_distance = simhash1.distance(simhash2)
            print(f"hamming_distance: {hamming_distance}")
            Hamming += hamming_distance
        return ["{:.4f}".format(precision/num), "{:.4f}".format(recall/num), "{:.4f}".format(f1/num), "{:.4f}".format(bleu/num), "{:.4f}".format(Cosine/num), "{:.4f}".format(perplexity/num), "{:.4f}".format(Hamming/num), "{:.4f}".format(METEOR/num)]

"""这个是向其中写入数据的函数"""
def appendExcle(data):  # openpyxl库储存数据到excel
    import openpyxl as op
    from openpyxl import load_workbook
    wb = load_workbook("测试.xlsx")  # 创建工作簿对象
    ws = wb.worksheets[0] # 创建子表
    ws.append(data)
    wb.save("测试.xlsx")
def createExcel():
    import openpyxl as op
    wb = op.Workbook()
    ws = wb['Sheet']
    ws.append(title)
    wb.save("测试.xlsx")
def deletelast():
    import openpyxl as op
    from openpyxl import load_workbook
    wb = load_workbook("测试.xlsx")  # 创建工作簿对象
    ws = wb.worksheets[0]  # 创建子表
    ws.delete_rows(ws.max_row)
    ws.save("测试.xlsx")
"""四个参数的变化大小(必须用小数)"""
top_p_change = 0.2
temperature_change = 0.4
present_penalty_change = 1.0
frequency_penalty_change = 1.0
title = ['top_p', 'temperature', 'presence_penalty', 'frequency_penalty','precision','recall','bleu','Cosine','perplexity','Hamming','METEOR']
import pandas as pd
# 指定 Excel 文件路径
file_path = '测试.xlsx'  # 替换为你的 Excel 文件路径
# 读取 Excel 文件
df = pd.read_excel(file_path)
try:
    f = open("测试.xlsx")
    f.close()
except FileNotFoundError:
    #要是没有这个文件就创建一个
    createExcel()
if df.empty:
    #如果文件中没有之前的数据则从最小值开始
    top_p = 0
    temperature = 1
    present_penalty = -2.0
    frequency_penalty = -2.0
else:
    #从上一次的值之后开始继续
    last_row = df[['top_p', 'temperature', 'presence_penalty', 'frequency_penalty']].iloc[-1].tolist()
    top_p = last_row[0]
    temperature = last_row[1]
    present_penalty = last_row[2]
    frequency_penalty = last_row[3] + frequency_penalty_change
    if frequency_penalty > 2.0:
        frequency_penalty = -2.0
print(f"提取的四个属性分别为:{top_p, temperature, present_penalty,frequency_penalty}")


import numpy as np
while top_p <= 1.0:
    while temperature <= 2.0:
        while present_penalty <=2.0:
            while frequency_penalty <=2.0:
                text(top_p, temperature, present_penalty, frequency_penalty)
                list = [str("{:.1f}".format(top_p)), str("{:.1f}".format(temperature)), str(present_penalty),str(frequency_penalty)]
                for i in data():
                    list.append(i)
                print(list)
                # appendExcle(list)
                # print(
                #     f"评判完毕，四个系数分别为top_p:{top_p},temperature:{temperature},present_penalty:{present_penalty},frequency_penalty:{frequency_penalty}")
                frequency_penalty += frequency_penalty_change
            present_penalty += present_penalty_change
            frequency_penalty = -2.0
        temperature  += temperature_change
        present_penalty = -2.0
    temperature = 0.0
    top_p += top_p_change
# top_p = 0
# temperature = 0.8
# present_penalty = -0.5
# frequency_penalty = -0.5
# text(top_p, temperature, present_penalty, frequency_penalty)
# list = [str("{:.1f}".format(top_p)), str("{:.1f}".format(temperature)), str(present_penalty),str(frequency_penalty)]
# for i in data():
#     list.append(i)
# appendExcle(list)
# print(list)
